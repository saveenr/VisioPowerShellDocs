...
| `New-VisioPoint` | Constructs a `Point` value &mdash; e.g. `New-VisioPoint 4 5`. Used by many `New-VisioShape` examples. |
| `New-VisioRectangle` | Constructs a `Rectangle` value &mdash; `New-VisioRectangle Left Bottom Right Top`. Used by `New-VisioShape -Rectangle`/`-Oval`. |
