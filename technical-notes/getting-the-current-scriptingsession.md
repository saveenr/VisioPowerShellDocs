...
From there you can call into any of the `VisioScripting` command groups &mdash; `Document`, `Page`, `Selection`, `View`, `Application`, etc. &mdash; or its `Client.Application` property to reach the raw `IVisio.Application` COM object.

## See also

* [Use VisioAutomation](use-visioautomation.md) &mdash; calling into the bundled `VisioAutomation` library directly.
* [`Get-VisioClient`](../cmdlets/other-cmdlets.md) &mdash; reference (covered in the Other cmdlets note).
